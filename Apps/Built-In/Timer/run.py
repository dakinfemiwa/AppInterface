...
class runApp:
    def __init__(self, instance, appName):
        ...
        self.Clock = Label(self.AppScreen, text="00:00:00.0", font=("Segoe UI", self.FontArray[3]), bg="#262626", fg="white")
        self.Clock.place(relx=.0, rely=.0, width=self.screenWidth, height=.4 * self.screenHeight)

        self.StartButton = Button(self.AppScreen, text=" ▶ ", command=lambda: self.startClock(), bg="black", fg="white",
                                  cursor="hand2", bd=0)
        self.StartButton.place(relx=.415, rely=.5, width=self.screenWidth * .05, height=self.screenHeight * .1)

        self.PauseButton = Button(self.AppScreen, text=" ⏸ ", command=lambda: self.pauseClock(), bg="black", fg="white",
                                  cursor="hand2", bd=0)
        self.PauseButton.place(relx=.475, rely=.5, width=self.screenWidth * .05, height=self.screenHeight * .1)

        self.StopButton = Button(self.AppScreen, text=" ⬛ ", command=lambda: self.endClock(), bg="black", fg="white",
                                 cursor="hand2", bd=0)
